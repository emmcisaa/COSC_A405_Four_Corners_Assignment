class Problem:
    ...
    def h(self, state):
        pacman_position, remaining_food_list = state
        if not remaining_food_list:
            return 0  # No food left, so cost is zero
        
        # Initialize visited dictionary and other variables
        mst = {food: False for food in remaining_food_list}
        sumSteps = 0
        ...
        while not all(mst.values()):
            ...
            # Find the nearest unvisited food dot
            for food in remaining_food_list:
                if not mst[food]:
                    steps = len(self.BFS(current_position, food))  # BFS returns the path length
                    if steps < min_steps:
                        min_steps = steps
                        closest_food = food
            
            # Update sumSteps and mark the food as visited
            sumSteps += min_steps
